...
diff --git a/src/components/HorseColorCalculator.tsx b/src/components/HorseColorCalculator.tsx
index c0a16e8684486ef6f56d846adc33e434b74a9943..7ff3517413f2099bf0a32f8354886238a6b9d4b8 100644
--- a/src/components/HorseColorCalculator.tsx
+++ b/src/components/HorseColorCalculator.tsx
@@ -234,50 +234,63 @@ function Section({ title, subtitle, children, defaultOpen = true }: { title: str
       <CardContent className="p-0">
         <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-6 py-4 text-left">
           <div>
             <div className="text-sm uppercase tracking-wider text-gray-500">{title}</div>
 ...
 export default function HorseColorCalculator() {
   ...
   const [g, setG] = useState(defaultG());
   const phenotype = useMemo(() => computePhenotype(g), [g]);
+  const baseColor = useMemo(() => baseCoat(g.E, g.A), [g.E, g.A]);
+  const horseLayers = useMemo(() => {
+    const baseMap = {
+      Black: "/horse/base-black.png",
+      Bay: "/horse/base-bay.png",
+    } as const;
+    const base = baseMap[baseColor];
+    return {
+      base,
+      mane: "/horse/mane-black.png",
+      tail: "/horse/tail-black.png",
+    };
+  }, [baseColor]);
...
--- a/src/components/HorseColorCalculator.tsx
+++ b/src/components/HorseColorCalculator.tsx
@@ -359,33 +372,45 @@ export default function HorseColorCalculator() {
                 <div className="mt-5 space-y-2">
                   {phenotype.alerts.map((h, i) => (
                     <div key={i} className="flex items-start gap-2 text-sm text-rose-700 bg-rose-50/70 border border-rose-200 rounded-xl px-3 py-2">
                       <AlertTriangle className="h-4 w-4 mt-0.5"/>
...
+
+            <Section title="Color Preview" defaultOpen={true}>
+              {horseLayers.base ? (
+                <div className="relative w-full h-64">
+                  <img src={horseLayers.base} className="absolute inset-0 w-full h-full object-contain" alt="horse base" />
+                  <img src={horseLayers.mane} className="absolute inset-0 w-full h-full object-contain" alt="horse mane" />
+                  <img src={horseLayers.tail} className="absolute inset-0 w-full h-full object-contain" alt="horse tail" />
+                </div>
+              ) : (
+                <div className="text-sm text-gray-500">No preview available for this combination.</div>
+              )}
+            </Section>
           </div>
         </div>
